...
namespace ImGui
{
    ...
    IMGUI_API void SetCursorPos(const ImVec2 &local_pos, float d);                          //    are using the main, absolute coordinate system.
    ...
    // Widgets: Color TextEditor/Picker (tip: the ColorEdit* functions have a little color square that can be left-clicked to open a picker, and right-clicked to open an option menu.)
...
struct ImGuiIO
{
    ...
    // Optional: Notify OS Input Method TextEditor of the screen position of your cursor for text input position (e.g. when using Japanese/Chinese IME on Windows)
...
// (Optional) Support for IME (Input Method TextEditor) via the io.SetPlatformImeDataFn() function.
